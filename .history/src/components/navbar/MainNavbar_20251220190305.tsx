        ...
        {session?.user ? (
          <>
            ...
            <Link
              ...
              className="accountBtn hideOnMobile"
            >
              <Settings size={18} /> Account
            </Link>
            <button onClick={handleLogout} className="logoutBtn hideOnMobile">
              <LogOut size={18} /> Logout
            </button>
          </>
        ) : (
          <Link href="/login" className="loginBtn hideOnMobile">
            <User /> Login
          </Link>
        )}

        <Link href="/ContactForm" className="contactBtn hideOnMobile">
          <Mail /> Send Message
        </Link>
      </div>
    </nav>
